...
if __name__ == "__main__":
    ...
    project_name = "bzp"
    # project_name = "crypto-algorithms"
    files = preprocess(f"data/{project_name}", ["include", "src"],
    macros = {"ALWAYS_INLINE": "inline", "STATIC": "static", "HIDDEN": "", "TARGET_ATTRIBUTE_AUTO":"",
    ...
    with open(f"./output/{project_name}/files.json", "w") as f:
        f.write(json.dumps(metadata, default=lambda o: o.__dict__(), indent=4, ensure_ascii=False))
    with open(f"./output/{project_name}/functions.json", "w") as f:
