pub fn VOS_AVL3_Insert_Or_Find(mut pstTree: Ptr<AVL3_TREE>, mut pstNode: Ptr<AVL3_NODE>, mut pstTreeInfo: Ptr<AVL3_TREE_INFO>) -> Ptr<Void> {
    ...
    while (pstParentNode != AVL_NULL_PTR!()).as_bool() {
        iResult = (pstTreeInfo.pfCompare)((pstNode.cast::<Ptr<u8>>() + iKeyOffset).cast::<Ptr<Void>>().cast(), 
                                         (pstParentNode.cast::<Ptr<u8>>() + iKeyOffset).cast::<Ptr<Void>>().cast()).cast();
        if iResult > 0 {
            if (pstParentNode.pstRight != AVL_NULL_PTR!()).as_bool() {
                pstParentNode = pstParentNode.pstRight.cast();
                continue;
            }
            VosAvlNodeRightInsert(pstTree.cast::<Ptr<AVLBASE_TREE_S>>(), pstParentNode.cast(), pstNode.cast());
        } else if iResult < 0 {
            if (pstParentNode.pstLeft != AVL_NULL_PTR!()).as_bool() {
                pstParentNode = pstParentNode.pstLeft.cast();
                continue;
            }
            VosAvlNodeLeftInsert(pstTree.cast::<Ptr<AVLBASE_TREE_S>>(), pstParentNode.cast(), pstNode.cast());
        } else {
            pstNode.sRHeight = -1;
            pstNode.sLHeight = -1;
    ...
    VosAvlBalanceTree(pstTree.cast::<Ptr<AVLBASE_TREE_S>>(), pstParentNode.cast());
