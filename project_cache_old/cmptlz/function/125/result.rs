pub fn CmptlzDpProcess(mut encCtx: Ptr<CmptLzEncCtx>, mut mf: Ptr<CmptMfCtx>, mut mainReps: Ptr<u32>, mut lenEnd: u32, mut position: u32, mut cur: u32) -> u32 {
    ...
    let mut mainRepIndex: u32 = 0;
    c_for!(; mainRepIndex < CMPTLZ_NUM_REPS!(); mainRepIndex.suffix_plus_plus(); {
        ...
        let mut lenEqual: u32 = CmptMemCmpLenSafe(buf.cast(), bufRepBack.cast(), CMPTLZ_MATCH_LEN_MIN!(), bufAvail.cast());
