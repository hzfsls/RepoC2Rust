pub fn CmptLzRepDec(mut decCtx: Ptr<CmptLzDecCtx>, mut pRange: Ptr<u32>, mut pRangeCode: Ptr<u32>, mut pRangeBound: Ptr<u32>, mut dicPosLimit: usize, mut posState: u32) -> u32 {
    ...
    let mut rangeBound: u32 = *pRangeBound.cast();
    ...
    *pRangeBound = rangeBound.cast();
    ...
    repLen = CmptLzLenDec(decCtx.cast(), probSlot.cast(), pRange.cast(), pRangeCode.cast(), pRangeBound.cast(), posState.cast());

    return CmptLzDecByDistAndLen(decCtx.cast(), repDist.cast(), (repLen + 2).cast(), dicPosLimit.cast());
}
