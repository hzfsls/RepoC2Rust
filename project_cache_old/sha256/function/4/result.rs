pub fn vosSha256Hash(mut pucData: Ptr<u8>, mut uiLen: u32, mut pstCtx: Ptr<VOS_SHA256_CTX>) {
    ...
    if (pucSrc == NULL!()).as_bool() || (uiLenTmp == 0).as_bool() || (pstCtx == NULL!()).as_bool() || (pstCtx.corrupted == 1).as_bool() ||
        (pstCtx.computed == 1).as_bool() || (vosSha256CtxPrepare(pstCtx.cast(), uiLenTmp.cast()) != SHA256_OK!()).as_bool() {
        return;
    }
