pub fn CmptlzDpInit(mut encCtx: Ptr<CmptLzEncCtx>, mut mf: Ptr<CmptMfCtx>, mut position: u32) -> u32 {
    ...
    if mf.readAhead == 0 {
        lenMain = CmptlzMatchFinder(mf.cast(), c_ref!(matchesCount).cast(), encCtx.matches.cast());
    } else {
    ...
    let bufAvail: u32 = CMPTLZ_FIND_MIN!(CmptMfAvail(mf.cast()) + 1, CMPT_MF_LONGEST_MATCH!());
    ...
    c_for!(; i < CMPTLZ_NUM_REPS!(); i.suffix_plus_plus(); {
        ...
        repLens[i] = CmptMemCmpLenSafe(buf.cast(), bufBack.cast(), CMPTLZ_MATCH_LEN_MIN!(), bufAvail.cast());
    ...
    encCtx.opts[1].price = (CmptPriceBit0(encCtx.cast(), encCtx.isMatch[encCtx.state][posState].cast()) +
    ...
    let matchPrice: u32 = CmptPriceBit1(encCtx.cast(), encCtx.isMatch[encCtx.state][posState].cast());
    ...
    c_for!(let mut i: u32 = 0; i < CMPTLZ_NUM_REPS!(); i.suffix_plus_plus(); {
