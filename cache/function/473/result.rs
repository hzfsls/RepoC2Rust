pub fn RapidlzCompWithExternalDict(mut strmCtx: Ptr<RapidlzStreamCtx>, mut src: Ptr<u8>, mut dest: Ptr<u8>, mut srcSize: i32, mut destSize: i32) -> i32 {
    ...
    loop {
        ...
        loop {
            ...
            forwardPos += jumpStep;
            jumpStep = (searchMatchNb.suffix_plus_plus() >> RAPIDLZ_STEP_FORWARD_BASE!()).cast();
        ...
        token = curDest.cast();
        RAPIDLZ_RETURN_IF_NOT_TRUE!(RapidlzStreamEncLiterals(curSrc.cast(), curSrcAnchor.cast(), c_ref!(curDest).cast(), destEnd.cast()), RAPIDLZ_ENC_NOT_OK!());
        ...
        if prefixDictStart == dict {
            ...
            curSrcMatchEnd = RapidlzCompressExpandBackward(srcLimitOnlyWithDict.cast(), (match + RAPIDLZ_MIN_MATCH!()).cast(), (curSrc + RAPIDLZ_MIN_MATCH!()).cast()).cast();
        ...
        if (matchOffset >= prefixDictLimit) && (matchOffset + RAPIDLZ_MAX_OFFSET!() >= current) {
            if RAPIDLZ_READ32BIT!(curSrc) == RAPIDLZ_READ32BIT!(match) {
                token = curDest.suffix_plus_plus();
                *token = 0;
                offset = (current - matchOffset).cast();
