    ...
    const uint8_t *buf2, uint32_t len, uint32_t limit)
{
    ...
    uint32_t safeLimit = (limit < CMPTLZ_MEM_COMPARE_SAFE_DIST) ? 0 : (limit - CMPTLZ_MEM_COMPARE_SAFE_DIST);
    while (len < safeLimit) { // 防止结尾读越界
        const uint64_t x = *(const uint64_t *)(buf1 + len) - *(const uint64_t *)(buf2 + len);
        if (x != 0) {
...
static ALWAYS_INLINE uint32_t CmptMemCmpLen(const uint8_t *buf1, const uint8_t *buf2, uint32_t len, uint32_t limit)
{
    ...
    while (len < limit) {
        ...
        if (x != 0) {
            len += (uint32_t)__builtin_ctzll(x) >> 3;
            return CMPTLZ_FIND_MIN(len, limit);
        }
        len += 8;
    }
    return limit;
