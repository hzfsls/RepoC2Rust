pub fn RapidlzCompWithPrefixDict(mut strmCtx: Ptr<RapidlzStreamCtx>, mut src: Ptr<u8>, mut dest: Ptr<u8>, mut srcSize: i32, mut destSize: i32) -> i32 {
    ...
    loop {
        ...
        RAPIDLZ_EXPAND_FORWARD!(prefixDictStart.cast(), match.cast(), curSrc.cast(), curSrcAnchor.cast());
        ...
        #[cfg(RAPIDLZ_DEBUG)]
        if RAPIDLZ_UNLIKELY!(RAPIDLZ_LIT_AND_MATCH_COPY_END!(curDest.cast(), matchLen) > destEnd) {
            return RAPIDLZ_ENC_NOT_OK!();
        }
