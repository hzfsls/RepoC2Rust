pub fn CmptPriceGenLenTable(mut encCtx: Ptr<CmptLzEncCtx>, mut lenEncoder: Ptr<CmptLenEncoder>) {
    ...
    if i > (1 << CMPT_LEN_LOW_BITS!()) * CMPT_DOUBLE!() {
        ...
        c_do!({
            ...
            prob = probs[(i + (1 << (CMPT_LEN_HIGH_BITS!() - 1))).cast()].cast();
            prices[(i * CMPT_DOUBLE!()).cast()] = price + CmptPriceBit0(encCtx.cast(), prob.cast()).cast();
            prices[(i * CMPT_DOUBLE!() + 1).cast()] = price + CmptPriceBit1(encCtx.cast(), prob.cast()).cast();
        } while i);
