pub fn CmptRcDistProcess(mut encCtx: Ptr<CmptLzEncCtx>, mut posSlot: u32, mut dist: u32) -> i32 {
    ...
    if dist < CMPT_DIST_LIMIT_2!() {
        ...
        c_do!({
            ...
            pos2 += pos2;
            CMPT_RC_NORMALIZE!(encCtx.rcCtx.cast(), range.cast(), shiftRes.cast());
            CMPTLZ_RETURN_IF_NOT_OK!(shiftRes);
        ...
        c_for!(k = 0; k < CMPTLZ_ALIGN_BITS!() - 1; k.suffix_plus_plus(); {
            bit = dist & 1;
            dist >>= 1;
            CMPT_RC_BIT_PROCESS!(encCtx.rcCtx.cast(), (encCtx.probAlign + m).cast(), bit.cast(), bit0Prob.cast(), range.cast(), newBound.cast(), shiftRes.cast());
            CMPTLZ_RETURN_IF_NOT_OK!(shiftRes);
            m = (m << 1) + bit;
        });
        bit = dist & 1;
