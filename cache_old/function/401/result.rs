pub fn CmptlzCompress(mut src: Ptr<Void>, mut srcSize: usize, mut dst: Ptr<Void>, mut dstSize: Ptr<usize>, mut param: Ptr<CmptlzCompParam>) -> i32 {
    ...
    return CmptlzEncode(dst.cast::<Ptr<u8>>(), dstSize.cast(), src.cast::<Ptr<u8>>(), srcSize.cast(), c_ref!(props).cast(), param.protData.cast(), c_ref!(param.protSize).cast(), endMarker.cast(), alloc.cast());
}
