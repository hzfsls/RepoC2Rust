pub fn CmptLzLitDec(mut decCtx: Ptr<CmptLzDecCtx>, mut pRange: Ptr<u32>, mut pRangeCode: Ptr<u32>, mut pRangeBound: Ptr<u32>) -> u32 {
    ...
    let mut litPosMask: u32 = ((0x100 << decCtx.prop.litPos) - (0x100 >> litCtx)).cast();
    ...
    if (procPos != 0).as_bool() || (checkDicSize != 0).as_bool() {
        probSlot += (CMPTLZ_REP3!() * ((((procPos << 8) + dict[(if dictPos == 0 { dictBufSize } else { dictPos }) - 1]) & litPosMask) << litCtx)).cast();
    }
