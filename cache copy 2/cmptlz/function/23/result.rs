pub fn CmptlzDecompress(mut src: Ptr<Void>, mut srcSize: usize, mut dst: Ptr<Void>, mut dstSize: Ptr<usize>, mut param: Ptr<CmptlzDecParam>) -> i32 {
    if (src == NULL!() || dst == NULL!() || dstSize == NULL!()).as_bool() {
        CMPTLZ_LOG!(CMPT_ERROR_UNSUPPORTED!(), cstr!("The input parameter NULL is incorrect."));
        return CMPT_ERROR_UNSUPPORTED!();
    }

    if (srcSize > 0x7fffffff || *dstSize > 0x7fffffff).as_bool() {
        CMPTLZ_LOG!(CMPT_ERROR_UNSUPPORTED!(), cstr!("dstSize:0x{} srcSize:0x{}"), *dstSize, srcSize);
        return CMPT_ERROR_UNSUPPORTED!();
    }

    if (param == NULL!() || param.memHook == NULL!() || param.protData == NULL!() || param.protSize != CMPTLZ_PROPS_SIZE!()).as_bool() {
    ...
    let mut ret: i32 = CmptLzDecode(c_ref!(decIn).cast(), c_ref!(decOut).cast(), param.protData.cast(), CMPTLZ_FINISH_ANY!(), c_ref!(enFinStat).cast(), param.memHook.cast());
